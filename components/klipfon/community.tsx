...
function Avatar({profile,large=false}:{profile:CommunityProfile;large?:boolean}){
  return <div aria-hidden="true" className={'community-avatar '+(large?'large ':'')+(profile.role==='creator'?'creator':levelFor(profile.views).tone)}>{profile.name.trim().split(/\s+/).slice(0,2).map(n=>n[0]).join('').toLocaleUpperCase('tr-TR')}</div>;
}
export function CommunityProfileCard({profile:p}:{profile:CommunityProfile}){
  ...
  return <a className={'community-card '+(creator?'creator':level.tone)} href={'/topluluk/'+encodeURIComponent(p.id)}>
    ...
    <Avatar profile={p}/><div className="community-name"><h3>{p.name}</h3><CheckCircle2 size={18} aria-label="Onaylı hesap"/></div>
    <span className={'pill '+(creator?'pill-green':'')}>{creator?'Onaylı yayıncı':level.name}</span>
...
export function Community(){
  ...
  return <><Header/><main className="wrap community-main"><section className="community-hero"><div><div className="eyebrow"><Users size={15}/> KLİPFON TOPLULUĞU</div><h1>Birlikte üret.<br/><span className="accent-text">Kliplerin iz bıraksın.</span></h1><p>Klipperları ve yayıncıları tanı. Klipleri keşfet, doğrulanmış sonuçları gör, birlikte büyü.</p><div className="action-row"><a href="/panel?bolum=profile" className="btn">Topluluktaki yerini al <ArrowUpRight size={16}/></a><a href="#uyeler" className="btn btn-secondary">Üyeleri keşfet</a></div></div><div className="community-summary"><ShieldCheck size={30}/><strong>Üreten bir topluluk.</strong><p>Her profil onaylı.<br/>Her başarı gerçek kayıtlara bağlı.</p><div><span><b>{data?count(data.summary?.clippers||0):'—'}</b> Klipper</span><span><b>{data?count(data.summary?.creators||0):'—'}</b> Yayıncı</span></div></div></section>
    ...
    <div aria-live="polite" className="community-results-label">{loading?'Üyeler yükleniyor…':`${count(data?.total||0)} onaylı profil`}</div>
    {error?<div className="error" role="alert">{error}</div>:loading?<div className="community-grid" aria-label="Yükleniyor">{[1,2,3].map(n=><div className="community-placeholder" key={n}/>)}</div>:data?.profiles.length?<div className="community-grid">{data.profiles.map(p=><CommunityProfileCard key={p.id} profile={p}/>)}</div>:<InfoEmpty title={search?'Aradığın isim bulunamadı.':'Topluluk ilk üyelerini bekliyor.'} description={search?'Başka bir isim veya hesap türüyle tekrar ara.':'Profilini tamamla ve sosyal hesabını doğrulat. Onaylandığında burada kendi profilinle yer alacaksın.'}>{!search&&<a className="btn btn-secondary" href="/panel?bolum=profile">Profilimi tamamla</a>}</InfoEmpty>}
